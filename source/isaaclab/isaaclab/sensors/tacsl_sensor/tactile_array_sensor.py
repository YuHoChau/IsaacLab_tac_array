...
class TactileArraySensor(SensorBase):
    r"""A tactile sensor for tactile array finger.

    **Example Usage:**
        For a complete working example, see: ``scripts/demos/sensors/tactile_array_sensor.py``
    ...
    **Current Limitations:**
    ...
    cfg: TactileArraySensorCfg
    """The configuration parameters."""

    def __init__(self, cfg: TactileArraySensorCfg):
    ...
    def __str__(self) -> str:
        ...
        render_info = (
            f"{self.cfg.render_cfg.base_data_path}/{self.cfg.render_cfg.sensor_data_dir_name}"
            if self.cfg.render_cfg is not None
            else "None"
        )
        return (
    ...
    def _initialize_force_field(self):
        ...
        self._generate_tactile_points(
            ...
            tactile_points_distance=getattr(self.cfg, "tactile_points_distance", 0.001),
    ...
    def _generate_tactile_points(self, num_divs: list, tactile_points_distance: float, visualize: bool):
        ...
        Args:
            ...
            tactile_points_distance: Distance between adjacent tactile points in meters.
        ...
        # Use the directly specified distance between adjacent tactile points
        tactile_points_dx = tactile_points_distance 
        ...
        # Create normal axis vector for dot product projection (unit vector in slim_axis direction)
        self._tactile_normal_axis = torch.eye(3, dtype=torch.float32, device=self._device)[slim_axis]
    ...
    def _compute_tactile_forces_from_sdf(
        ...
        if collision_mask.any() or self.cfg.visualize_sdf_closest_pts:
            ...
            if self._camera_sensor is None:
                # Extract normal and shear components using dot product projection
                # Project force onto normal axis direction
                tactile_normal_force[:] = torch.sum(
                    tactile_force_tactile * self._tactile_normal_axis.view(1, 1, 3),
                    dim=-1
                )
                fn_component = tactile_normal_force.unsqueeze(-1) * self._tactile_normal_axis.view(1, 1, 3)
                ft_component = tactile_force_tactile - fn_component
                tactile_shear_force[:] = ft_component[..., :2]
